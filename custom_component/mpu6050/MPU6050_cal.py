...
pidax = SimplePID(0, -15000, 15000, kp, ki, kd, 50, True)
piday = SimplePID(0, -15000, 15000, kp, ki, kd, 50, True)
pidaz = SimplePID(0, -15000, 15000, kp, ki, kd, 50, True)
pidgx = SimplePID(0, -15000, 15000, kp, ki, kd, 50, True)
pidgy = SimplePID(0, -15000, 15000, kp, ki, kd, 50, True)
pidgz = SimplePID(0, -15000, 15000, kp, ki, kd, 50, True)

accel_reading = mpu.get_acceleration()

x_accel_reading = accel_reading.x
y_accel_reading = accel_reading.y
z_accel_reading = accel_reading.z
...
x_accel_offset_avg = [0]*l
y_accel_offset_avg = [0]*l
z_accel_offset_avg = [0]*l
...
x_gyro_reading = gyro_reading.x
y_gyro_reading = gyro_reading.y
z_gyro_reading = gyro_reading.z

x_gyro_avg = [0]*l
y_gyro_avg = [0]*l
z_gyro_avg = [0]*l

x_gyro_offset_avg = [0]*l
y_gyro_offset_avg = [0]*l
z_gyro_offset_avg = [0]*l
...
try:
    while True:
        ...
        # Gyro calibration
        # gyros[gxindex] = gyro_reading
        # gxindex += 1
        # if gxindex == len(x_gyro_avg):
        #     # average gyros:
        #     avg_gyro_x = avg_from_array([g.x for g in gyros])
        #     avg_gyro_y = avg_from_array([g.y for g in gyros])
        #     avg_gyro_z = avg_from_array([g.z for g in gyros])
        #     print(f'Gyro averages - X: {avg_gyro_x}, Y: {avg_gyro_y}, Z: {avg_gyro_z}')
